...
def _maybe_compile_torch_model(torch, model, device_label: str):
    # FIX: Triton-Compiler für Windows komplett überspringen
...
class DeckLinkLiveOutput:
    ...
    def _frame_to_bgra(self, frame):
        # FIX: Direkte Konvertierung zur Minimierung der CPU-Last
        if frame.ndim == 3 and frame.shape[2] == 3:
            return cv2.cvtColor(frame, cv2.COLOR_RGB2BGRA)
        return frame # Falls schon BGRA/RGBA
...
class DeckLinkLiveInput:
    ...
    def open(self):
        ...
        # --- FIX: Auto-Detect komplett deaktivieren, um "Flapping/Blinken" zu stoppen! ---
        self._input_flags = 0  # 0 entspricht bmdVideoInputFlagDefault
        # ---------------------------------------------------------------------------------
    ...
    def _on_video_frame(self, video_frame):
        try:
            ...
            if pixel_format == self._decklink_api.bmdFormat8BitBGRA:
                ...
                print(f"🚨 DECKLINK FEHLER: Unerwartetes Pixelformat vom ATEM empfangen (Format-ID: {pixel_format})")
                return

        except Exception as e:
            print(f"🚨 DECKLINK CRASH beim Entpacken: {e} (Width: {width}, Height: {height}, RowBytes: {row_bytes})")
            return
...
class RVMByteDanceModel:
    ...
    def predict_mask(self, rgb_frame):
        ...
        try:
            ...
            # Temporalen Cache leeren, wenn sich die Aufloesung aendert
        if getattr(self, '_last_shape', None) != (model_h, model_w):
                self.rec = [None, None, None, None]
                self._last_shape = (model_h, model_w)

            # FIX: Kurzzeitgedächtnis für absolut latenzfreies Tracking komplett abschalten!
        self.rec = [None, None, None, None]
...
class FoolproofSyncApp:
    ...
    def _apply_live_main_ai_input_size(self, size):
        ...
        try:
            ...
            pass
        try:
            self.model_status_label.configure(text=f"{self.model_status}\n{self._main_ai_auto_tune_note}")
        except Exception:
            pass

    def _maybe_auto_tune_main_ai_input_size(self, source_fps, processed_fps, avg_total_ms):
        # FIX 1: Auto-Tuning komplett deaktiviert, damit die 4090 auf 1024px bleibt!
        return
    ...
    def _start_main_ai_workers(self, session_id):
        batch_size = 1 # FIX: Kein Batching, sofortige Verarbeitung
    ...
    def update_gui_loop(self):
        ...
        self.root.after(100, self.update_gui_loop) # FIX: UI-Refresh auf 10 FPS senken (spart GPU/CPU)
    ...
    def video_worker_loop(self):
        while self.is_running:
            ...
            # FIX 2: Volle 1080p Auflösung an die KI geben, statt das verkleinerte UI-Bild!
            rgb_frame = cv2.cvtColor(frame, cv2.COLOR_BGR2RGB)

            # FIX 3: Frame-Stau verhindern. Wenn die KI rechnet, werfen wir neue Bilder sofort weg.
            # Das garantiert 0 Millisekunden Puffer-Latenz und ein absolut direktes Tracking!
