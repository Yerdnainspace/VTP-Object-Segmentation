...
class DeckLinkLiveOutput:
    ...
    def _write_decklink_frame(self, decklink_output, frame):
        ...
        video_frame = decklink_output.CreateVideoFrame(
            self.width, self.height, self.width * 4, self._body_api.bmdFormat8BitBGRA if hasattr(self, '_body_api') else self._decklink_api.bmdFormat8BitBGRA,
    ...
    def _on_video_format_changed(self, new_display_mode, detected_signal_flags):
        ...
        with self._format_lock:
            try:
                ...
                pixel_format = self._decklink_api.bmdFormat8BitBGRA if int(detected_signal_flags) & int(rgb_flag) else self._decklink_api.bmdFormat8BitYUV
...
class BiRefNetModel:
    def __init__(self, force_device=None):
        ...
        if missing_modules:
            raise RuntimeError(f"BiRefNet benoetigt zusaetzliche Pakete. Fehlend: {', '.join(missing_modules)}.")
        ...
        if force_device == "cpu":
            ...
            self.device_label = "CPU"
            self.device_hint = "BiRefNet wurde manuell auf CPU gesetzt."
        elif force_device == "cuda":
            if not torch.cuda.is_available(): raise RuntimeError("CUDA nicht verfuegbar.")
        ...
        # 4090 Optimization: Torch Compile directly onto Ada Lovelace Arch
        ...
        try: self.model_dtype = next(self.model.parameters()).dtype
        except StopIteration: self.model_dtype = torch.float32

        self.mean = torch.tensor([0.485, 0.456, 0.406], device=self.device, dtype=self.model_dtype).view(1, 3, 1, 1)
        self.std = torch.tensor([0.229, 0.224, 0.225], device=self.device, dtype=self.model_dtype).view(1, 3, 1, 1)

        try:
            dummy = torch.zeros((1, 3, 512, 512), device=self.device, dtype=self.model_dtype)
            with torch.inference_mode(): _ = self.model(dummy)
        except Exception: pass
        ...
        if isinstance(output, dict):
            for key in ("logits", "preds", "prediction", "out"):
                if key in output:
                    output = output[key]
                    break
            else: output = next(iter(output.values()))
...
class RVMByteDanceModel:
    def __init__(self, force_device=None):
        try: import torch
        except ImportError as exc: raise RuntimeError("RVM benoetigt PyTorch.") from exc
...
def create_segmentation_model(model_name, mediapipe_model_path, force_device=None):
    if model_name == "MediaPipe Selfie": return MediaPipeSelfieModel(mediapipe_model_path)
    if model_name == "BiRefNet": return BiRefNetModel(force_device=force_device)
    if model_name == "RVM ByteDance": return RVMByteDanceModel(force_device=force_device)
...
class YoloObjectDetector:
    def __init__(self, model_name="yolo11n-seg.pt", force_device=None):
        ...
        missing_modules = [name for name in required_modules if importlib.util.find_spec(name) is None]
        if missing_modules: raise RuntimeError("YOLO benoetigt ultralytics.")
        ...
        if force_device == "cpu":
            self.device = torch.device("cpu")
            self.device_label = "CPU"
            self.device_hint = "YOLO laeuft auf CPU."
        else:
            self.device, self.device_label, self.device_hint = _select_torch_device(torch)
        with quiet_terminal_output():
    ...
    def detect(self, rgb_frame, conf=0.35, max_det=12, imgsz=416):
        with quiet_terminal_output():
            results = self.model.predict(source=rgb_frame, conf=float(conf), max_det=int(max_det), imgsz=int(imgsz), device=self.predict_device, verbose=False)
        ...
        if masks_obj is not None and getattr(masks_obj, "data", None) is not None:
            try: masks = masks_obj.data.detach().float().cpu().numpy()
            except Exception: masks = None
        detections = []
        h, w = rgb_frame.shape[:2]
        for i, box in enumerate(xyxy):
            ...
            if masks is not None and i < len(masks):
                mask = masks[i]
                if mask.shape[:2] != (h, w): mask = cv2.resize(mask, (w, h), interpolation=cv2.INTER_LINEAR)
                mask = np.clip(mask.astype(np.float32, copy=False), 0.0, 1.0)
            detections.append({"name": name, "class_id": class_id, "conf": float(confs[i]), "box": (x1, y1, x2, y2), "mask": mask})
...
class CorridorKeyRefiner:
    def __init__(self, device_mode="Automatisch", img_size=CORRIDORKEY_IMG_SIZE):
        ...
        missing_modules = [name for name in required_modules if importlib.util.find_spec(name) is None]
        if missing_modules: raise RuntimeError("CorridorKey-Pakete fehlen.")
    ...
    def refine(self, rgb_frame, alpha_2d, despill_strength=0.7, despeckle_size=400):
        ...
        result = self.engine.process_frame(rgb_frame, mask, input_is_linear=False, despill_strength=despill_strength, auto_despeckle=True, despeckle_size=despeckle_size, generate_comp=False, post_process_on_gpu=self.device == "cuda", screen_channel=1)
...
class FoolproofSyncApp:
    def __init__(self, root):
        self.root = root
        self.root.title("AI Segmenter - Multi Model Optimized")
        ...
        self.segmenter = create_segmentation_model(self.model_name.get(), self.model_path, self._resolve_main_ai_force_device())
    ...
    def _format_model_status(self, segmenter, choice):
        if choice == "YOLO":
            with self.yolo_model_lock: detector = self.yolo_detector
            if detector is not None:
                base = f"Modell bereit: YOLO ({detector.device_label})"
                if detector.device_hint: return base + "\n" + str(detector.device_hint)
                return base
    ...
    def setup_gui(self):
        ...
        ctk.CTkLabel(control_panel, text="Main AI Hardware", font=ctk.CTkFont(size=12, weight="bold")).pack(pady=(4, 0))
        ...
        ctk.CTkLabel(control_panel, text="YOLO Objektauswahl", font=ctk.CTkFont(size=14, weight="bold")).pack(pady=(12, 5))
        self.chk_yolo_enabled = ctk.CTkCheckBox(control_panel, text="YOLO-Nachbearbeitung aktiv", variable=self.yolo_enabled, command=self.toggle_yolo)
        self.chk_yolo_enabled.pack(pady=(0, 6), padx=20, anchor="w")
        self.yolo_model_select = ctk.CTkOptionMenu(control_panel, values=YOLO_MODEL_OPTIONS, variable=self.yolo_model_name, command=self.change_yolo_model)
        self.yolo_model_select.pack(pady=(0, 6), padx=10, fill="x")
        ctk.CTkLabel(control_panel, text="YOLO Hardware", font=ctk.CTkFont(size=12, weight="bold")).pack(pady=(4, 0))
        self.yolo_device_select = ctk.CTkOptionMenu(control_panel, values=YOLO_DEVICE_OPTIONS, variable=self.yolo_device_mode, command=self.change_yolo_device)
        self.yolo_device_select.pack(pady=(3, 6), padx=10, fill="x")
        ctk.CTkLabel(control_panel, text="YOLO Confidence", font=ctk.CTkFont(size=12, weight="bold")).pack(pady=(4, 0))
        self.yolo_conf_slider = ctk.CTkSlider(control_panel, from_=0.1, to=0.8, number_of_steps=14, variable=self.yolo_confidence)
        self.yolo_conf_slider.pack(pady=(3, 6), padx=10, fill="x")
        self.chk_yolo_sync_postprocess = ctk.CTkCheckBox(control_panel, text="YOLO synchron nach Main AI", variable=self.yolo_sync_postprocess)
        self.chk_yolo_sync_postprocess.pack(pady=(0, 6), padx=20, anchor="w")
        self.chk_yolo_select_all = ctk.CTkCheckBox(control_panel, text="Neue Objekte automatisch auswaehlen", variable=self.yolo_select_all)
        self.chk_yolo_select_all.pack(pady=(0, 6), padx=20, anchor="w")
        self.yolo_status_label = ctk.CTkLabel(control_panel, textvariable=self.yolo_status, wraplength=300, justify="left")
        ...
        ctk.CTkLabel(self.live_frame, text="Output Sync", font=ctk.CTkFont(size=12, weight="bold")).pack(pady=(8, 0), padx=10, anchor="w")
        self.sync_overlay_select = ctk.CTkOptionMenu(self.live_frame, values=["Aus", "Frame", "Timecode", "Beides"], variable=self.sync_overlay_mode)
    ...
    def _resolve_yolo_force_device(self, for_postprocess=False):
        ...
        if mode == "CPU": return "cpu"
        if mode == "CUDA": return None
        return "cpu" if for_postprocess else None

    def _yolo_device_mode_note(self, for_postprocess=False):
        mode = self.yolo_device_mode.get()
        if mode == "Automatisch": return "Auto: CPU" if for_postprocess else "Auto: CUDA"
        return mode

    def _reset_yolo_runtime_state(self, clear_post_detector=False, clear_primary_detector=False):
        with self.yolo_async_lock: self.yolo_async_pending_frame = None
        with self.yolo_model_lock:
            if clear_post_detector: self.yolo_post_detector = None
            if clear_primary_detector: self.yolo_detector = None
        self.yolo_detections = []
    ...
    def _load_yolo_worker(self, model_name):
        try:
            ...
            with self.yolo_model_lock: self.yolo_detector = detector
            status = f"YOLO bereit: {model_name} ({detector.device_label})"
            if detector.device_hint: status += "\n" + str(detector.device_hint)
            ...
            with self.yolo_model_lock: self.yolo_detector = None
            self.root.after(0, lambda e=exc: self.yolo_status.set(f"YOLO Fehler: {e}"))

    def _load_yolo_post_worker(self):
        try:
            detector = YoloObjectDetector(YOLO_POSTPROCESS_MODEL, force_device=self._resolve_yolo_force_device(True))
            with self.yolo_model_lock: self.yolo_post_detector = detector
            ...
            with self.yolo_model_lock: self.yolo_post_detector = None
    ...
    def _refresh_yolo_detection_list(self):
        if not hasattr(self, "yolo_objects_frame"): return
        for child in self.yolo_objects_frame.winfo_children(): child.destroy()
        self.yolo_detection_vars = {}
        ...
        if hasattr(self, "corridor_header"): pack_options["before"] = self.corridor_header
        elif hasattr(self, "metrics_header"): pack_options["before"] = self.metrics_header
        ...
        if not self.yolo_detections:
            ...
            ctk.CTkLabel(self.yolo_objects_frame, text=text, justify="left", wraplength=280).pack(anchor="w", fill="x")
            return
        ...
        for det in self.yolo_detections:
            ...
            box = ctk.CTkCheckBox(self.yolo_objects_frame, text=label, variable=var, command=lambda k=key: self._toggle_yolo_detection(k))
            box.pack(pady=2, anchor="w")

    def _toggle_yolo_detection(self, key):
        var = self.yolo_detection_vars.get(key)
        if var is not None and bool(var.get()): self.yolo_selected_keys.add(key)
        else: self.yolo_selected_keys.discard(key)

    def _yolo_detection_signature(self, detections):
        return tuple((det.get("key"), int(det.get("class_id", -1))) for det in detections)
    ...
    def _apply_yolo_detections(self, detections, elapsed_ms, now=None):
        ...
        try: list_visible = bool(self.yolo_objects_frame.winfo_ismapped())
        except Exception: list_visible = True
        ...
        if ui_due:
            self.yolo_last_ui_update_time = now
            self.root.after(0, lambda count=len(detections), ms=elapsed_ms, selected=selected_count: self.yolo_status.set(f"YOLO aktiv: {count} Objekte, {selected} ausgewaehlt\nAnalyse: {ms:.1f} ms"))
        return elapsed_ms

    def _run_yolo_primary(self, rgb_frame):
        with self.yolo_model_lock: detector = self.yolo_detector
        if detector is None: return 0.0
    ...
    def _schedule_yolo_postprocess(self, rgb_frame):
        if not self.yolo_enabled.get() or self._is_yolo_primary_model(): return 0.0
        with self.yolo_model_lock: detector = self.yolo_post_detector
        if detector is None: return 0.0

        self.yolo_frame_counter += 1
        now = time.perf_counter()
        if self.yolo_frame_counter % int(self.yolo_detect_every) != 1: return 0.0
        if now - self.yolo_last_detect_time < float(self.yolo_min_detect_interval): return 0.0
        self.yolo_last_detect_time = now

        with self.yolo_async_lock:
            self.yolo_async_pending_frame = rgb_frame.copy()
            if self.yolo_async_running: return 0.0
    ...
    def _run_yolo_postprocess_sync(self, rgb_frame):
        ...
        self.yolo_last_detect_time = now
        start = time.perf_counter()
        try: detections = detector.detect(rgb_frame, conf=float(self.yolo_confidence.get()), max_det=8, imgsz=320)
        except Exception as exc:
            self.root.after(0, lambda e=exc: self.yolo_status.set(f"YOLO Fehler: {e}"))
            detections = []
        elapsed_ms = (time.perf_counter() - start) * 1000.0
    ...
    def _yolo_postprocess_worker(self):
        while True:
            with self.yolo_async_lock:
                ...
                self.yolo_async_pending_frame = None
            if frame is None:
                with self.yolo_async_lock: self.yolo_async_running = False
                return
            ...
            start = time.perf_counter()
            try: detections = detector.detect(frame, conf=float(self.yolo_confidence.get()), max_det=8, imgsz=320)
            except Exception as exc:
                self.root.after(0, lambda e=exc: self.yolo_status.set(f"YOLO Fehler: {e}"))
                detections = []
            elapsed_ms = (time.perf_counter() - start) * 1000.0
    ...
    def _make_yolo_roi(self, shape):
        h, w = shape[:2]
        if not self.yolo_enabled.get(): return None
        with self.yolo_model_lock: detector_ready = self.yolo_post_detector is not None
        if not detector_ready or not self.yolo_detections: return None
        mask = np.zeros((h, w), dtype=np.float32)
        selected = set(self.yolo_selected_keys)
        for det in self.yolo_detections:
            if det["key"] not in selected: continue
            x1, y1, x2, y2 = det["box"]
    ...
    def _make_yolo_alpha(self, shape):
        ...
        for det in self.yolo_detections:
            ...
            if det_mask is not None:
                if det_mask.shape[:2] != (h, w): det_mask = cv2.resize(det_mask, (w, h), interpolation=cv2.INTER_LINEAR)
        ...
        if np.max(alpha) > 0: alpha = cv2.GaussianBlur(alpha, (7, 7), 0)
        return np.clip(alpha, 0.0, 1.0).astype(np.float32, copy=False)

    def _draw_yolo_overlay(self, frame_rgb):
        if not self._yolo_selection_active() or not self.yolo_detections: return frame_rgb
        out = frame_rgb.copy()
        selected = set(self.yolo_selected_keys)
        for det in self.yolo_detections:
            x1, y1, x2, y2 = det["box"]
    ...
    def _postprocess_to_alpha(self, rgb_frame: np.ndarray, mask_u8: np.ndarray) -> np.ndarray:
        """
        4090 OPTIMIZATION: Executes complete edge refining, morphology and blurring
        directly inside the GPU VRAM via PyTorch CUDA. Reduces CPU bottleneck to 0.
        """
        h, w = rgb_frame.shape[:2]
        ...
        import torch
        use_cuda = torch.cuda.is_available() and self.main_ai_device_mode.get() in ("CUDA", "Automatisch")

        if use_cuda:
            with torch.inference_mode():
                mask_t = torch.from_numpy(mask_u8).to("cuda", non_blocking=True).float()
                alpha_raw = mask_t / 255.0
                bin_mask = (mask_t >= 128).float().unsqueeze(0).unsqueeze(0) # [1, 1, H, W]
                
                # Morphological Close simulation via GPU MaxPooling
                import torch.nn.functional as F
                bin_mask = F.max_pool2d(bin_mask, kernel_size=5, stride=1, padding=2)
                bin_mask = -F.max_pool2d(-bin_mask, kernel_size=5, stride=1, padding=2)
                
                erode_size = int(self.edge_erode.get())
                if erode_size > 0:
                    erode_size = _ensure_odd_ksize(erode_size, min_k=1)
                    bin_mask = -F.max_pool2d(-bin_mask, kernel_size=erode_size, stride=1, padding=erode_size // 2)
                
                soft_size = _ensure_odd_ksize(int(self.edge_soft.get()), min_k=1)
                try:
                    import torchvision.transforms.functional as TVF
                    alpha_core = TVF.gaussian_blur(bin_mask, kernel_size=[soft_size, soft_size])[0, 0]
                except Exception:
                    # High performance fallback cascade inside VRAM
                    alpha_core = bin_mask[0, 0]
                    for _ in range(3):
                        alpha_core = F.avg_pool2d(alpha_core.unsqueeze(0).unsqueeze(0), kernel_size=max(3, soft_size//2), stride=1, padding=max(3, soft_size//2)//2)[0, 0]

                core_gate = (alpha_core > 0.03).float()
                alpha = torch.clamp(alpha_raw * core_gate, 0.0, 1.0)
                alpha = torch.maximum(alpha, alpha_core * 0.85)
                return alpha.cpu().numpy()
        else:
            alpha_raw = mask_u8.astype(np.float32) / 255.0
            bin_mask = ((mask_u8 >= 128).astype(np.uint8) * 255)
    ...
    def update_bg_button_state(self, *args):
        self.btn_load_bg.configure(state="normal" if self.bg_mode.get() == "CustomImage" else "disabled")
    ...
    def _make_display_image(self, rgb_frame: np.ndarray, alpha_2d: np.ndarray, processed_frame: np.ndarray) -> Image.Image:
        ...
        if view_mode == "Input": return Image.fromarray(self._draw_yolo_overlay(rgb_frame))
    ...
    def _make_view_frame(self, rgb_frame: np.ndarray, alpha_2d: np.ndarray, processed_frame: np.ndarray) -> np.ndarray:
        ...
        if view_mode == "Alpha Matte": return cv2.cvtColor(self._alpha_to_u8(alpha_2d), cv2.COLOR_GRAY2RGB)
    ...
    def refresh_decklink_devices(self):
        ...
        def worker():
            try:
                ...
                inputs = run_with_timeout(get_decklink_input_devices, [], timeout=5.0)
                error = None
            except Exception as exc:
                values, inputs, error = None, [], exc
    ...
    def _finish_decklink_refresh(self, values, inputs=None, error=None):
        ...
        non_decklink_sources = [source for source in existing_sources if source and not str(source).startswith("DeckLink: ") and source != "Keine Live-Quelle gefunden"]
    ...
    def _update_live_output_sync_status(self):
        ...
        lines.append(f"Sync: Overlay {self.sync_overlay_mode.get()} | Fill +{int(self.fill_delay_frames.get())}F | Matte +{int(self.matte_delay_frames.get())}F")
        self.live_output_status.set("\n".join(lines))

    def _current_output_fps(self):
        mode = self.live_output_mode.get()
        return float(DECKLINK_OUTPUT_MODES[mode][3]) if mode in DECKLINK_OUTPUT_MODES else 25.0
    ...
    def _sync_overlay_text(self, frame_index, stream_label):
        ...
        if mode == "Aus": return None
        parts = [stream_label]
        if mode in ("Frame", "Beides"): parts.append(f"F{int(frame_index):06d}")
        if mode in ("Timecode", "Beides"): parts.append(self._format_output_timecode(frame_index))
    ...
    def _draw_output_sync_overlay(self, frame_rgb, frame_index, stream_label):
        ...
        cv2.rectangle(out, (x - 8, y - th - baseline - 8), (x + tw + 8, y + baseline + 8), (0, 0, 0), -1)
    ...
    def start_live_output(self):
        ...
        try:
            ...
            if self.live_key_output_enabled.get():
                ...
                if key_device_name == device_name and self.live_output_enabled.get(): raise RuntimeError("Fill und Matte brauchen getrennte Ausgaenge.")
            ...
            self.decklink_output, self.decklink_key_output = None, None
            self.live_output_enabled.set(False)
    ...
    def select_post_input(self):
        ...
        file_path = filedialog.askopenfilename(title="Datei fuer Postproduktion waehlen", filetypes=[("Video/Bild", "*.mp4;*.mov;*.avi;*.mkv;*.jpg;*.jpeg;*.png;*.bmp;*.webp"), ("Videos", "*.mp4;*.mov;*.avi;*.mkv"), ("Bilder", "*.jpg;*.jpeg;*.png;*.bmp;*.webp"), ("Alle Dateien", "*.*")])
    ...
    def select_post_output(self):
        ...
        file_path = filedialog.asksaveasfilename(title="Speicherziel waehlen", initialfile=os.path.basename(initial), initialdir=os.path.dirname(initial) if os.path.dirname(initial) else None,  defaultextension=ext if ext else ".mp4", filetypes=[("Apple ProRes 4444 MOV", "*.mov"), ("MP4 Video", "*.mp4"), ("PNG Bild", "*.png"), ("JPEG Bild", "*.jpg"), ("Alle Dateien", "*.*")])
    ...
    def _default_post_output_path(self, input_path):
        ...
        return base + "_processed.mov" if self.bg_mode.get() == "Transparent" else base + "_processed.mp4"
    ...
    def _compose_processed_frame(self, rgb_frame, alpha_2d, bg_cache=None, last_bg_mode=None):
        """
        4090 OPTIMIZATION: Heavy alpha composition matrix operations have been migrated 
        to CUDA Tensors to free up CPU threads during high-res file rendering.
        """
        h, w = rgb_frame.shape[:2]
        ...
        if use_cuda:
            with torch.inference_mode():
                fg_t = torch.from_numpy(fg_rgb).to("cuda", non_blocking=True).float()
                bg_t = torch.from_numpy(bg_cache).to("cuda", non_blocking=True).float()
                alpha_t = torch.from_numpy(alpha_2d).to("cuda", non_blocking=True).float().unsqueeze(-1)
                
                out_t = fg_t * alpha_t + bg_t * (1.0 - alpha_t)
                output_rgb = out_t.clamp(0, 255).byte().cpu().numpy()
        else:
    ...
    def _process_post_rgb_frame(self, rgb_frame, segmenter, bg_cache=None, last_bg_mode=None):
        if self._is_yolo_primary_model():
            ...
            if self.yolo_sync_postprocess.get(): self._run_yolo_postprocess_sync(rgb_frame)
            else: self._schedule_yolo_postprocess(rgb_frame)
    ...
    def _process_post_image(self, input_path, output_path):
        ...
        output_to_write = cv2.cvtColor(output_frame, cv2.COLOR_RGBA2BGRA) if output_frame.shape[2] == 4 else cv2.cvtColor(output_frame, cv2.COLOR_RGB2BGR)
        os.makedirs(os.path.dirname(output_path), exist_ok=True) if os.path.dirname(output_path) else None
        if not cv2.imwrite(output_path, output_to_write): raise RuntimeError("Ausgabebild Fehler.")
    ...
    def _find_ffmpeg_executable(self):
        ...
        candidates = [os.path.join(app_dir, "ffmpeg.exe"), os.path.join(app_dir, "ffmpeg", "bin", "ffmpeg.exe"), os.path.join(app_dir, "bin", "ffmpeg.exe")]
    ...
    def _open_prores_4444_writer(self, output_path, width, height, fps):
        ...
        if not ffmpeg_path: raise RuntimeError("FFmpeg fehlt im Pfad/Projektordner.")
        command = [ffmpeg_path, "-y", "-loglevel", "error", "-f", "rawvideo", "-pix_fmt", "rgba", "-s:v", f"{int(width)}x{int(height)}", "-r", f"{float(fps):.6f}", "-i", "pipe:0", "-an", "-c:v", "prores_ks", "-profile:v", "4", "-pix_fmt", "yuva444p10le", "-alpha_bits", "16", "-vendor", "apl0", output_path]
        try: return subprocess.Popen(command, stdin=subprocess.PIPE, stdout=subprocess.DEVNULL, stderr=subprocess.PIPE)
        except Exception as exc: raise RuntimeError(f"FFmpeg Startfehler: {exc}")
    ...
    def _process_post_transparent_video(self, input_path, output_path):
        ...
        if width <= 0 or height <= 0:
            cap.release()
            raise RuntimeError("Aufoesung ungueltig.")

        os.makedirs(os.path.dirname(output_path), exist_ok=True) if os.path.dirname(output_path) else None
        writer = self._open_prores_4444_writer(output_path, width, height, fps)

        with self.model_lock: segmenter = self.segmenter
        bg_cache, last_bg_mode, processed = None, None, 0
        start = time.perf_counter()
        ...
        try:
            while True:
                ...
                output_frame, alpha_2d, bg_cache, last_bg_mode = self._process_post_rgb_frame(rgb_frame, segmenter, bg_cache, last_bg_mode)
                ...
                except Exception as exc: raise RuntimeError(f"FFmpeg Pipeline-Fehler: {exc}")
        ...
        if writer.wait() != 0: raise RuntimeError(f"ProRes-Export fehlgeschlagen: {stderr.strip()}")
    ...
    def _process_post_video(self, input_path, output_path):
        ...
        cap = cv2.VideoCapture(input_path)
        if not cap.isOpened(): raise RuntimeError("Video ungueltig.")
        ...
        if width <= 0 or height <= 0:
            cap.release()
            raise RuntimeError("Aufloesung ungueltig.")

        os.makedirs(os.path.dirname(output_path), exist_ok=True) if os.path.dirname(output_path) else None
        writer = cv2.VideoWriter(output_path, cv2.VideoWriter_fourcc(*"mp4v"), fps, (width, height))
        if not writer.isOpened():
            cap.release()
            raise RuntimeError("VideoWriter Initialisierungsfehler.")
    ...
    def show_metrics_info(self):
        ...
        text = (
            ...
            "Latenz gesamt: Durchschnittliche Zeit fuer einen kompletten Frame von Kamera-Read bis fertigem Ausgabebild.\n\n"
            "Main AI: Reine Modellzeit fuer die Haupt-Maskenberechnung.\n\n"
            "Eingehend: Geschaetzte Rohdatenrate der Kamera.\n\n"
    ...
    def _reset_perf_metrics(self, source_fps=0.0, backend_name="-", capture_size=(0, 0)):
        ...
        with self.metrics_lock:
            self.metrics_text = "Performance\nMesse Daten ..." if self.is_running else "Performance\nKamera gestoppt"
    ...
    def _update_perf_metrics(self, frame_shape, total_ms, infer_ms, post_ms, compose_ms):
        ...
        if self._yolo_selection_active():
            ...
            if self.yolo_last_analysis_ms <= 0: text += "\nYOLO Analyse: wartet"
            elif self._is_yolo_primary_model(): text += f"\nYOLO Analyse: {self.yolo_last_analysis_ms:.1f} ms"
            elif self.yolo_sync_postprocess.get(): text += f"\nYOLO sync: {self.yolo_last_analysis_ms:.1f} ms"
    ...
    def _load_model_worker(self, choice, restart_camera):
        try:
            if choice == "YOLO":
                detector = YoloObjectDetector(self.yolo_model_name.get(), force_device=self._resolve_yolo_force_device(False))
                with self.yolo_model_lock: self.yolo_detector = detector
                with self.model_lock: self.segmenter = None
                status = f"Modell bereit: YOLO ({detector.device_label})"
                if detector.device_hint: status += "\n" + str(detector.device_hint)
                self.model_status = status
                self.root.after(0, lambda: self.yolo_status.set(f"YOLO als Hauptmodell aktiv: {self.yolo_model_name.get()} ({detector.device_label})"))
            else:
                new_segmenter = create_segmentation_model(choice, self.model_path, self._resolve_main_ai_force_device())
    ...
    def _finish_model_load(self, choice, restart_camera, error):
        ...
        if error is None:
            ...
            if choice != "YOLO" and not self.yolo_enabled.get():
                ...
                self.yolo_status.set("YOLO-Nachbearbeitung aus")
                self._refresh_yolo_detection_list()
            self.video_label.configure(image=self.empty_dummy_image, text="Postproduktion bereit" if self.app_mode.get() == "Postproduktion" else "Kamera gestoppt")
    ...
    def _load_image_from_path(self, file_path):
        if file_path and os.path.exists(file_path):
            ...
            except Exception as img_e: print(f"Bildfehler: {img_e}")
    ...
    def change_camera(self, choice):
        ...
        except Exception as e: print(f"Kamerawechsel Fehler: {e}")
    ...
    def _start_camera_internal(self, preserve_preview=False):
        ...
        if not preserve_preview:
            self.latest_pil_image, self.latest_display_payload = None, None
        ...
        if self.cap is not None and self.cap.isOpened():
            ...
            actual_fps = measured_fps if measured_fps > 1.0 else (reported_fps if reported_fps > 1.0 and reported_fps <= 240.0 else 0.0)
    ...
    def _stop_camera_internal(self, preserve_preview=False):
        ...
        if not preserve_preview: self.latest_pil_image, self.latest_display_payload = None, None
        self._reset_perf_metrics()
    ...
    def video_worker_loop(self):
        ...
        while self.is_running:
            ...
            try:
                ...
                if self._is_yolo_primary_model():
                    self._run_yolo_primary(rgb_frame)
                    mask_binary, alpha_2d = None, self._make_yolo_alpha(rgb_frame.shape)
                else:
                    with self.model_lock: segmenter = self.segmenter
                    mask_binary = segmenter.predict_mask(rgb_frame)
                ...
                self.is_running = False
                self.root.after(0, lambda e=model_error: self.video_label.configure(image=self.empty_dummy_image, text=f"Modellfehler: {e}"))
                break
