...
class FoolproofSyncApp:
    ...
    def select_post_output(self):
        ...
        filetypes = [("MP4 Video", "*.mp4"), ("PNG Bild", "*.png"), ("JPEG Bild", "*.jpg"), ("Alle Dateien", "*.*")]
    ...
    def _process_post_rgb_frame(self, rgb_frame, segmenter, bg_cache=None, last_bg_mode=None):
        ...
        return self._compose_processed_frame(rgb_frame, alpha_2d, bg_cache, last_bg_mode)
    ...
    def _process_post_image(self, input_path, output_path):
        ...
        output_rgb, _, _ = self._process_post_rgb_frame(rgb_frame, segmenter)
        ...
        preview = Image.fromarray(cv2.resize(output_rgb, (self.ui_w, self.ui_h), interpolation=cv2.INTER_AREA))
        self.latest_pil_image = preview
    ...
    def _process_post_video(self, input_path, output_path):
        ...
        try:
            while True:
                ...
                output_rgb, bg_cache, last_bg_mode = self._process_post_rgb_frame(
    ...
    def video_worker_loop(self):
        ...
        while self.is_running:
            ...
            if current_mode != last_bg_mode or self.force_bg_update:
                if current_mode == "Checker":
                    bg_cache = self._get_checker_background(self.ui_w, self.ui_h)
                elif current_mode == "Green":
                    bg_cache = np.zeros((self.ui_h, self.ui_w, 3), dtype=np.uint8)
                    bg_cache[:] = (0, 255, 0)
                elif current_mode == "CustomImage":
                    bg_cache = self._get_custom_background(self.ui_w, self.ui_h)
                else:
