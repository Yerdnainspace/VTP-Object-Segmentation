    ...
    def flush_if_due(self, force=False):
        with self.lock:
            ...
            row = {
                ...
                "birefnet_input_size": self.metadata.get("birefnet_input_size", ""),
...
MODEL_OPTIONS = ["MediaPipe Selfie", "BiRefNet", "BiRefNet TensorRT", "RVM ByteDance", "YOLO"]
MAIN_AI_DEVICE_OPTIONS = ["Automatisch", "CUDA", "CPU"]
BIREFNET_INPUT_SIZE_OPTIONS = ["512", "448", "384", "320"]
...
class BiRefNetModel:
    def __init__(self, force_device=None, use_tensorrt=False, input_size=512):
        ...
        # Prefer FP16 on CUDA. DirectML is intentionally kept FP32 because many
        # BiRefNet operators are not reliable in half precision there.
        ...
        # Warmup to reduce "first frame takes forever" effect on GPU. CPU warmup
        # can take minutes, so the first real frame pays that cost instead.
...
def create_segmentation_model(model_name, mediapipe_model_path, force_device=None, birefnet_input_size=512):
    if model_name == "MediaPipe Selfie":
        return MediaPipeSelfieModel(mediapipe_model_path)
    if model_name == "BiRefNet":
        return BiRefNetModel(force_device=force_device, input_size=birefnet_input_size)
    if model_name == "BiRefNet TensorRT":
        return BiRefNetModel(force_device=force_device, use_tensorrt=True, input_size=birefnet_input_size)
...
class FoolproofSyncApp:
    def __init__(self, root):
        ...
        self.birefnet_input_size = ctk.StringVar(value="512")
        self.model_lock = threading.Lock()
        self.segmenter = create_segmentation_model(
            self.model_name.get(),
            self.model_path,
            self._resolve_main_ai_force_device(),
            self._current_birefnet_input_size()
        )
    ...
    def _current_birefnet_input_size(self):
        try:
            return int(self.birefnet_input_size.get())
        except Exception:
            return 512
    ...
    def setup_gui(self):
        ...
        ctk.CTkLabel(control_panel, text="BiRefNet Groesse", font=ctk.CTkFont(size=12, weight="bold")).pack(
            pady=(4, 0))
        self.birefnet_size_select = ctk.CTkOptionMenu(
            control_panel,
            values=BIREFNET_INPUT_SIZE_OPTIONS,
            variable=self.birefnet_input_size,
            command=self.change_birefnet_input_size
        )
        self.birefnet_size_select.pack(pady=(3, 6), padx=10, fill="x")
    ...
    def _start_pipeline_profiler(self, source, backend_name, capture_size, source_fps):
        ...
        birefnet_input_size = getattr(segmenter, "input_size", "")
    ...
    def _format_pipeline_summary(self, summary):
        ...
        size = summary.get("birefnet_input_size", "")
        if model_backend:
            model_line += f" | {model_backend}"
        if size:
            model_line += f" | {size}px"
        ...
        return (
            ...
            f"Zeit: AI {ai_ms:.1f} ms | Alpha {alpha_ms:.1f} | Compose {compose_ms:.1f} | SDI {output_ms:.1f}\n"
            f"Total {total_ms:.1f} ms | Capture wait {capture_read_ms:.1f} ms\n"
    ...
    def change_birefnet_input_size(self, choice):
        if self.loaded_model_name not in ("BiRefNet", "BiRefNet TensorRT") and self.model_name.get() not in ("BiRefNet", "BiRefNet TensorRT"):
            return
        self.change_model(self.loaded_model_name)
    ...
    def video_capture_loop(self):
        while self.is_running and not self.pipeline_stop_event.is_set():
            ...
            if profiler is not None:
                profiler.sample("capture_read", read_done - read_start)
            ...
            with self.ai_frame_condition:
                ...
                self.ai_latest_frame = (self.ai_latest_frame_id, rgb_frame, frame.shape, read_start)
    ...
    def video_ai_loop(self):
        while self.is_running and not self.pipeline_stop_event.is_set():
            ...
            if item is None:
                continue
            frame_id, rgb_frame, original_shape, frame_start = item
            profiler = self.pipeline_profiler
            gpu_alpha_t = None
            gpu_segmenter = None
            try:
            ...
            if profiler is not None:
                ...
                profiler.flush_if_due()
            self._update_perf_metrics(original_shape, total_ms, infer_ms, post_ms, compose_ms)
